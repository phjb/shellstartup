...
[ScriptBlock]$CmderPrompt = {
   ...
   # Write-Host -NoNewline "$([char]0x200B)"
    Microsoft.PowerShell.Utility\Write-Host "paulo in " -NoNewLine -ForegroundColor Green
    Microsoft.PowerShell.Utility\Write-Host $p -NoNewLine -ForegroundColor Blue
    if (get-command git -erroraction silentlycontinue) {
        checkGit($pwd.ProviderPath)
    }
    Microsoft.PowerShell.Utility\Write-Host "`n❯ " -NoNewLine -ForegroundColor Green
}
